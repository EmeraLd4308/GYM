...
import type { WeeklySbdRpeRow, ProfileSbdMaxKg } from "@/features/stats/lib/weekly-rpe";
...
import type { ApproachMapHints, ApproachMapPoint } from "@/features/stats/lib/approach-map";
...
const ApproachMapChart = dynamic(
  () =>
    import("@/features/stats/components/ApproachMapChart").then((m) => ({
      default: m.ApproachMapChart,
    })),
  {
    loading: () => (
      <div className="h-80 animate-pulse rounded-2xl bg-zinc-900/50 ring-1 ring-white/[0.05]" />
    ),
    ssr: false,
  },
);
...
export function StatsLazyCharts({
  ...
  approachMapPoints: ApproachMapPoint[];
  approachMapHints: ApproachMapHints;
  approachMapProfileMaxKg: ProfileSbdMaxKg;
}) {
  return (
    <>
      ...
      <div className="mt-8">
        <ApproachMapChart
          points={approachMapPoints}
          hints={approachMapHints}
          profileMaxKg={approachMapProfileMaxKg}
        />
      </div>
    </>
  );
