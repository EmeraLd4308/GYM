...
import { inferWorkoutTagFromExercises } from "@/features/workouts/lib/workout-tags";
...
export async function recalculateUserDerivedMetricsFromProfile(userId: string): Promise<void> {
  ...
  const updates: { id: string; rpe: Prisma.Decimal | null }[] = [];
  for (const set of sets) {
    ...
    updates.push({ id: set.id, rpe: next != null ? new Prisma.Decimal(next) : null });
  }
