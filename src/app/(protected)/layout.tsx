...
export default async function ProtectedLayout({
  ...
  return (
    <div className="flex min-h-dvh flex-col">
      ...
      <div className="mx-auto flex min-h-0 w-full max-w-4xl flex-1 flex-col pb-[calc(4.5rem+env(safe-area-inset-bottom,0px))] pl-[max(1rem,env(safe-area-inset-left))] pr-[max(1rem,env(safe-area-inset-right))] pt-4 md:max-w-6xl md:pb-8 md:pl-[max(1.5rem,env(safe-area-inset-left))] md:pr-[max(1.5rem,env(safe-area-inset-right))] md:pt-8 xl:pl-[max(2rem,env(safe-area-inset-left))] xl:pr-[max(2rem,env(safe-area-inset-right))]">
        <OfflineWeekProvider>
